...
class PointGroup(LightningModule):
    def __init__(
        self,
        in_channels: int,
        num_classes: int,
        ...
        self.test_save_name = test_save_name + str(cls_global_weight) + str(cls_local_weight) + currtime
        ...
        self.cls_unet = SparseUNet.build(
            channels[0], channels[:2], block_repeat, norm_fn, without_stem=True
        )
        ...
        self.local_cls_unet = SparseUNet.build(
            channels[0], channels[:2], block_repeat, norm_fn, without_stem=True
        )
    ...
    def _training_or_validation_step(
        ...
        if sem_preds.sum() <= 0:
            return scene_ids, sem_seg, None, loss_sem_seg
        ...
        # loss
        self.log(f"{prefix}_loss/total_loss", loss, batch_size=batch_size, sync_dist=True,)
        self.log(
        ...
        self.log(
            f"{prefix}_loss/loss_pt_offset_dist",
            loss_pt_offset_dist,
            batch_size=batch_size, sync_dist=True,
        )
        self.log(
            f"{prefix}_loss/loss_pt_offset_dir",
            loss_pt_offset_dir,
            batch_size=batch_size, sync_dist=True,
        )
        self.log(
        ...
        self.log(
            f"{prefix}_loss/loss_local_cls",
            loss_local_cls,
            batch_size=batch_size, sync_dist=True,
        )
        self.log(
            f"{prefix}_loss/loss_global_cls",
            loss_global_cls,
            batch_size=batch_size, sync_dist=True,
        )
    ...
    def validation_step(self, point_clouds: List[PointCloud], batch_idx: int, dataloader_idx: int):
        ...
        #     proposals.gt_npcs = None
            proposals_ = Instances(score_preds=proposals.score_preds, pt_sem_classes=proposals.pt_sem_classes, \
        ...
        else: proposals_=proposals
        sem_seg_ = Segmentation(batch_size=sem_seg.batch_size) 
        if dataloader_idx > len(self.validation_step_outputs) - 1:
            self.validation_step_outputs.append([])
        self.validation_step_outputs[dataloader_idx].append((scene_ids, sem_seg_, proposals_))
        return scene_ids, sem_seg_, proposals_
    ...
    def on_validation_epoch_end(self):
        ...
        for i_, validation_step_outputs in enumerate(self.validation_step_outputs):
            ...
            # names = [x[0] for x in validation_step_outputs if x[2]!= None]
            ...
            # for name in names:
            #     pred_info = {}
            #     pred_info['conf'] = np.array([p.score_preds.cpu().numpy() for p in proposals if p != None])
            #     pred_info['label_id'] = np.array([p.pt_sem_classes.cpu().numpy() for p in proposals if p != None])
            #     pred_info['mask'] = np.array([p.valid_mask.cpu().numpy() for p in proposals if p != None])
            #     gt_file = os.path.join("/data2/haoran/data/GAPartNet_All_drawer_fixed_", split+ '_gt',
            #                             name + '.txt')
            #     gt2pred, pred2gt, npcs = eval.assign_instances_for_scan(name, pred_info, gt_file)
            #     matches[name] = {}
            #     matches[name]['gt'] = gt2pred
            #     matches[name]['pred'] = pred2gt
    ...
    def test_step(self, point_clouds: List[PointCloud], batch_idx: int, dataloader_idx: int):
        split = ["val", "intra", "inter"]
        scene_ids, sem_seg, proposals, _ = self._training_or_validation_step(
            point_clouds, batch_idx, split[dataloader_idx]
        )
        ...
        # if proposals != None:
        #     proposals.pt_sem_classes = proposals.sem_preds[proposals.proposal_offsets[:-1].long()]
        #     proposals.sorted_indices = proposals.sorted_indices
        #     proposals.valid_mask = None
        #     proposals.pt_xyz = None
        #     proposals.sem_preds = None
        #     proposals.npcs_preds = None
        ...
        if proposals != None:
            ...
            npcs_map =  torch.zeros_like(point_clouds[0].points[:,:3], device = sem_seg.sem_preds.device)
            result = Result(xyz = point_clouds[0].points[:,:3], rgb=point_clouds[0].points[:,3:], sem_preds=sem_seg.sem_preds, ins_preds = instance_label, npcs_preds=npcs_map)
        spl = split[dataloader_idx]
        scid = scene_ids[0]
        dir = self.test_save_name
        import os
        os.makedirs(f"/data3/haoran/GAPartNet_result/{dir}/{spl}", exist_ok=True)
        torch.save(result, f"/data3/haoran/GAPartNet_result/{dir}/{spl}/{scid}.pth")

        proposals = Instances(score_preds=proposals.score_preds, pt_sem_classes=proposals.pt_sem_classes, batch_indices=proposals.batch_indices, instance_labels=proposals.instance_sem_labels, ious=proposals.ious)
            
        return scene_ids, sem_seg, proposals

    def on_test_epoch_end(self, validation_step_outputs_list):
        splits = ["val", "intra", "inter"]
