...
class SparseUNet_raw(nn.Module):
    def __init__(self, stem: nn.Module, ublock: UBlock_raw):
        super().__init__()
...
class UBlock(nn.Module):
    def __init__(
        ...
        encoder_blocks = [
            block_fn(
                channels_down[0], channels_down[0], norm_fn, indice_key=f"subm{indice_key_id}"
            )
            for _ in range(block_repeat)
        ]
        self.encoder_blocks = spconv.SparseSequential(*encoder_blocks)

        if len(channels_down) > 1:
            self.downsample = spconv.SparseSequential(
                spconv.SparseConv3d(
                    channels_down[0], channels_down[1], kernel_size=2, stride=2,
                    bias=False, indice_key=f"spconv{indice_key_id}",
                ),
                norm_fn(channels_down[1]),
                nn.ReLU(),
            )

            self.ublock = UBlock(
                channels_down[1:],channels_up[1:], block_fn, block_repeat, norm_fn, indice_key_id + 1
            )

            self.upsample = spconv.SparseSequential(
                spconv.SparseInverseConv3d(
                    channels_up[1], channels_up[0], kernel_size=2,
                    bias=False, indice_key=f"spconv{indice_key_id}",
                ),
                norm_fn(channels_up[0]),
                nn.ReLU(),
            )

            decoder_blocks = [
                block_fn(
                    channels_up[0] + channels_down[0], channels_up[0], norm_fn,
                    indice_key=f"subm{indice_key_id}",
                ),
            ]
            for _ in range(block_repeat -1):
                decoder_blocks.append(
                    block_fn(
                        channels_up[0], channels_up[0], norm_fn,
                        indice_key=f"subm{indice_key_id}",
                    )
                )
            self.decoder_blocks = spconv.SparseSequential(*decoder_blocks)
...
class SparseUNet(nn.Module):
    ...
    def build(
        ...
        if not without_stem:
            stem = spconv.SparseSequential(
                spconv.SubMConv3d(
                    in_channels, channels_down[0], kernel_size=3,
                    padding=1, bias=False, indice_key="subm1",
                ),
                norm_fn(channels_down[0]),
                nn.ReLU(),
            )
        else:
            stem = spconv.SparseSequential(
        ...
        block = UBlock(channels_down, channels_up, ResBlock, block_repeat, norm_fn, \
            indice_key_id=1)
