...
class PointGroup(pl.LightningModule):
    def __init__(
        ...
        self.cls_head = Discriminator(input_dim=channels[0], num_domains=num_obj_cats)
    ...
    def forward_proposal_cls(
        ...
        # if reverse:
        #     pooled_cls_features = ReverseLayerF.apply(pooled_cls_features, alpha)
    ...
    def _training_or_validation_step(
        ...
        if self.current_epoch >= self.score_net_start_at and voxel_tensor is not None:
            ...
            if running_mode == "train" and num_points_per_instance is not None:
                
                cls_logits = self.forward_proposal_cls(
                    voxel_tensor, pc_voxel_id, proposals, reverse=self.reverse, alpha = self.alpha
                )
                loss_proposal_cls = self.loss_proposal_cls(cls_logits, proposals.cls_labels[proposals.proposal_offsets[:-1].long()].to(pt_xyz.device))
