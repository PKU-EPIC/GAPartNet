...
from .sparse_unet_new import SparseUNet
...
class PointGroup(pl.LightningModule):
    def __init__(
        ...
        self.sem_seg_head = nn.Linear(channels_up[0], num_classes)
        self.offset_head = nn.Sequential(
            nn.Linear(channels_up[0], channels_up[0]),
            norm_fn(channels_up[0]),
            nn.ReLU(inplace=True),
            nn.Linear(channels_up[0], 3),
        )

        self.score_unet = SparseUNet.build(
            channels_up[0], channels_down[:1], channels_down[:1], block_repeat, norm_fn, without_stem=True
        )
        self.score_head = nn.Linear(channels_up[0], num_classes - 1)
        ...
        self.npcs_head = nn.Linear(channels_up[0], 3 * (num_classes - 1))

        self.local_cls_head = Discriminator(input_dim=channels_up[0], num_domains=num_obj_cats)

        self.global_cls_head = Discriminator(input_dim=channels_up[0], num_domains=num_obj_cats)
    ...
    def forward_sem_seg(
        ...
        # print(voxel_features.features)
        # print(voxel_features.features.shape)
        # exit(123)
