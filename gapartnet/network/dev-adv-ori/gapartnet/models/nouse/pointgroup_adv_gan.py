...
class Counter():
    ...
    def update(self):
        self.num+=1
        if self.num%(2 * self.mol) < self.mol:
            return True
        else:
            return False
...
class PointGroup(pl.LightningModule):
    ...
    def forward_proposal_cls(
        ...
        cls_features = self.cls_unet(voxel_tensor)
        ...
        cls_logits = self.cls_head(pooled_cls_features)

        return cls_logits
    
    def loss_proposal_cls(
        self,
        ...
        # print(cls_logits.device)
        # print(cls_labels.device)
        if state == "cls":
            pass
        elif state == "reg":
            print(cls_labels)
        ...
        if state == "cls":
            ...
            print(cls_logits.shape)
            print(cls_logits)
            cls_score = F.softmax(cls_logits, dim = 1)
            print(cls_score)
            print(cls_score.shape)
            print(1/cls_logits.shape[1])
            loss = ((cls_score - 1/cls_logits.shape[1])**2).sum()/cls_logits.shape[0]
            print(loss)
        return loss
    ...
    def _training_or_validation_step(
        ...
        if self.current_epoch >= self.score_net_start_at and voxel_tensor is not None:
            ...
            if running_mode == "train":
                if state == "cls":
                    for param in self.cls_unet.parameters():
                ...
                cls_logits = self.forward_proposal_cls(
                    voxel_tensor, pc_voxel_id, proposals, reverse=self.reverse, alpha = self.alpha
                )
                loss_proposal_cls = self.loss_proposal_cls(cls_logits, proposals.cls_labels[proposals.proposal_offsets[:-1].long()].to(pt_xyz.device), state)

            else:
                loss_proposal_cls = 0.0
        ...
        loss += loss_prop_score + loss_prop_npcs + self.cls_weight * loss_proposal_cls
    ...
    def training_step(self, point_clouds: List[PointCloud], batch_idx: int):
        # mode = ""
        if self.counter.update() == True:
            mode = "cls"
        else:
            mode = "reg"
            
        _, _, _, loss = self._training_or_validation_step(
            point_clouds, batch_idx, "train", mode
        )
