...
from gapartnet.utils.info import OBJECT_NAME2ID
...
def apply_voxelization(
    ...
    voxel_features, voxel_coords, _, pc_voxel_id = voxelize(
        ...
        points_range_min=torch.as_tensor(points_range_min),
        points_range_max=torch.as_tensor(points_range_max),
        reduction="mean",
    )
...
def load_data(file_path: str):
    ...
    object_cat = OBJECT_NAME2ID[scene_id.split("_")[0]]
...
def from_folder(
    ...
    files = [load_data(path) for path in file_paths]
    print("finish loading data")
    files = [file for file in files if bool((file.instance_labels != -100).any()) ]
    files = [downsample(file, max_points=max_points) for file in files]
    files = [compact_instance_labels(file) for file in files]
    if augmentation:
        files = [apply_augmentations(file, 
            pos_jitter=pos_jitter,
            color_jitter=color_jitter,
            flip_prob=flip_prob,
            rotate_prob=rotate_prob,) for file in files]
    files = [generate_inst_info(file) for file in files]
    files = [file.to_tensor() for file in files]
    files = [apply_voxelization(file, voxel_size=voxel_size) for file in files]
...
class GAPartNetInst(lp.core.datamodule.LightningDataModule):
    def __init__(
        self,
        root_dir: str,
        max_points: int = 20000,
    ...
    def setup(self, stage: Optional[str] = None):
        if stage in (None, "fit", "validate", "test"):
            self.train_data_files = from_folder(
            ...
            self.val_data_files = from_folder(
            ...
            self.intra_data_files = from_folder(
            ...
            self.inter_data_files = from_folder(
